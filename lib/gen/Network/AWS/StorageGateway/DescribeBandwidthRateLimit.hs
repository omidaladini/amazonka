...
module Network.AWS.StorageGateway.DescribeBandwidthRateLimit where
...
instance AWSRequest DescribeBandwidthRateLimit where
    type Er DescribeBandwidthRateLimit = StorageGatewayError
...
data DescribeBandwidthRateLimitResponse = DescribeBandwidthRateLimitResponse
    { dbrlirsAverageDownloadRateLimitInBitsPerSec :: Maybe Integer
      -- ^ The average download bandwidth rate limit in bits per second. This field
      -- does not appear in the response if the download rate limit is not set.
    , dbrlirsAverageUploadRateLimitInBitsPerSec :: Maybe Integer
