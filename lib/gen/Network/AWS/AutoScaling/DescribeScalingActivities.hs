...
data DescribeScalingActivities = DescribeScalingActivities
    ...
    , dsateAutoScalingGroupName :: Maybe ResourceName
      -- ^ The name of the AutoScalingGroup.
    , dsateMaxRecords :: Maybe Int
      -- ^ The maximum number of scaling activities to return.
    , dsateNextToken :: Maybe Text
...
instance AWSRequest DescribeScalingActivities where
    ...
    request  = postQuery service "DescribeScalingActivities"
    response = responseXML

instance AWSPager DescribeScalingActivities where
    next rq rs
        | Just x <- dsaterNextToken rs = Just $ rq { dsateNextToken = Just x }
        | otherwise = Nothing

data DescribeScalingActivitiesResponse = DescribeScalingActivitiesResponse
    { dsaterActivities :: [Activity]
      -- ^ A list of the requested scaling activities.
    , dsaterNextToken :: Maybe Text
