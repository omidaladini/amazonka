...
data DescribePolicies = DescribePolicies
    { dptdAutoScalingGroupName :: Maybe ResourceName
      -- ^ The name of the Auto Scaling group.
    , dptdMaxRecords :: Maybe Int
      -- ^ The maximum number of policies that will be described with each call.
    , dptdNextToken :: Maybe Text
      -- ^ A string that is used to mark the start of the next batch of returned
      -- results for pagination.
    , dptdPolicyNames :: [ResourceName]
...
instance AWSRequest DescribePolicies where
    ...
    request  = postQuery service "DescribePolicies"
    response = responseXML

instance AWSPager DescribePolicies where
    next rq rs
        | Just x <- dptdrNextToken rs = Just $ rq { dptdNextToken = Just x }
        | otherwise = Nothing

data DescribePoliciesResponse = DescribePoliciesResponse
    { dptdrNextToken :: Maybe Text
      -- ^ A string that marks the start of the next batch of returned results.
    , dptdrScalingPolicies :: [ScalingPolicy]
