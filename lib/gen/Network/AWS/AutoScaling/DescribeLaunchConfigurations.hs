...
instance AWSRequest DescribeLaunchConfigurations where
    ...
    request  = postQuery service "DescribeLaunchConfigurations"
    response = responseXML

instance AWSPager DescribeLaunchConfigurations where
    next rq rs
        | Just x <- lcntdrNextToken rs = Just $ rq { lcntdNextToken = Just x }
        | otherwise = Nothing

data DescribeLaunchConfigurationsResponse = DescribeLaunchConfigurationsResponse
    { lcntdrLaunchConfigurations :: [LaunchConfiguration]
      -- ^ A list of launch configurations.
    , lcntdrNextToken :: Maybe Text
      -- ^ A string that marks the start of the next batch of returned results.
    } deriving (Eq, Show, Generic)
