...
resumeProcesses p1 = ResumeProcesses
    { spqdAutoScalingGroupName = p1
    , spqdScalingProcesses = []
    }

data ResumeProcesses = ResumeProcesses
    { spqdAutoScalingGroupName :: !ResourceName
      -- ^ The name or Amazon Resource Name (ARN) of the Auto Scaling group.
    , spqdScalingProcesses :: [Text]
...
instance AWSRequest ResumeProcesses where
    ...
    request  = postQuery service "ResumeProcesses"
