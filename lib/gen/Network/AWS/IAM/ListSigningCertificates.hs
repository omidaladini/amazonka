...
data ListSigningCertificates = ListSigningCertificates
    { lscdMarker :: Maybe Text
      -- ^ Use this only when paginating results, and only in a subsequent request
      -- after you've received a response where the results are truncated. Set it to
      -- the value of the Marker element in the response you just received.
    , lscdMaxItems :: Maybe Int
...
instance AWSRequest ListSigningCertificates where
    ...
    request  = postQuery service "ListSigningCertificates"
    response = responseXML

instance AWSPager ListSigningCertificates where
    next rq rs
        | Just x <- lscdrMarker rs = Just $ rq { lscdMarker = Just x }
        | otherwise = Nothing

data ListSigningCertificatesResponse = ListSigningCertificatesResponse
    { lscdrCertificates :: [SigningCertificate]
      -- ^ A list of the user's signing certificate information.
    , lscdrIsTruncated :: Maybe Bool
