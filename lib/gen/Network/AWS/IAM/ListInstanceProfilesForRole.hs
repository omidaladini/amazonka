...
listInstanceProfilesForRole :: Text
                            -> AWS (Either IAMError ListInstanceProfilesForRoleResponse)
listInstanceProfilesForRole p1 = undefined $ ListInstanceProfilesForRole
    { lipfrrRoleName = p1
    , lipfrrMarker = Nothing
    , lipfrrMaxItems = Nothing
    }
...
instance AWSRequest ListInstanceProfilesForRole where
    ...
    request = getQuery service "ListInstanceProfilesForRole"

instance AWSPager ListInstanceProfilesForRole where
    next rq rs
        | Just x <- lipfrrrsMarker rs = Just $ rq { lipfrrMarker = Just x }
        | otherwise = Nothing

data ListInstanceProfilesForRoleResponse = ListInstanceProfilesForRoleResponse
    { lipfrrrsInstanceProfiles :: [InstanceProfile]
      -- ^ A list of instance profiles.
    , lipfrrrsIsTruncated :: Maybe Bool
